...
@testset verbose = true "Pruebas de algoritmos de determinacion de frecuencia" begin
	...
	@testset "HFE para Triangular con $i" for i in casos
		...
		@test estimate_frequency(FE_HFE(fn = f, gr = g), s, Int64(round(1 + rand() * length(t))), fs) ≈ f rtol = 0.005
		...
		@test estimate_frequency(FE_HFE(fn = f, gr = g), sd, Int64(round(1 + rand() * length(t))), fs) ≈ fc rtol = 0.005
		...
		sc = s .+ 20
		@test estimate_frequency(FE_HFE(fn = f, gr = g), sc, Int64(round(1 + rand() * length(t))), fs) ≈ f rtol = 0.005
		# agrego ruido
		sr = s + abs(maximum(s) - minimum(s)) / 30 * rand(length(s))
		@test estimate_frequency(FE_HFE(fn = f, gr = g), sr, Int64(round(1 + rand() * length(t))), fs) ≈ f rtol = 0.05
